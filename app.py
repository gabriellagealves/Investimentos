...
if ticker:
    ...
    # 4.1 Crescimento
    st.subheader("4.1 Métricas de Crescimento")
    col1, col2, col3, col4 = st.columns(4)

    receita = info.get("totalRevenue", None)
    col1.metric("Receita (Revenue)", f"${receita/1e9:.1f}B" if receita else "N/D")

    lucro = info.get("netIncomeToCommon", None)
    col2.metric("Lucro Líquido", f"${lucro/1e9:.1f}B" if lucro else "N/D")

    ebitda = info.get("ebitda", None)
    col3.metric("EBITDA", f"${ebitda/1e9:.1f}B" if ebitda else "N/D")

    fcf = info.get("freeCashflow", None)
    col4.metric("Free Cash Flow (FCF)", f"${fcf/1e9:.1f}B" if fcf else "N/D")
